/ "drake/bindings/pydrake/common/text_logging_pybind.h"
#include <atomic>
#include <cstdlib>
#include <memory>
...
#include "drake/common/drake_assert.h"
  ...
  class pylogging_sink final
      ...
      // This is critically important to avoid deadlocks by lock order
      // inversion.
   ...
   protected:
    void sink_it_(const spdlog::details::log_msg& msg) final {
      ...
      // NOLINTNEXTLINE(build/namespaces_literals) This is how pybind11 wants
      // it.
      ...
      // fix up record.created, record.msecs, and record.relativeCreated, and
      // we'd still be brittle with respect to future changes to the record
      // class's internal bookkeeping. Instead, we'll allow the record's own
      // time from its constructor to survive.
  ...
  void MaybeRedirectPythonLogging() {
    ...
    // dist_sink) because the dist_sink takes a mutex on every log, which
    // suffers from lock priority order inversion once the GIL gets involved.
  ...
  void UseNativeCppLogging() {
    ...
    // After MaybeRedirectPythonLogging has happened, we expect two sinks. In
    // case it was opted-out of using the environment variable, we expect one
    // sink.
    std::vector<std::shared_ptr<spdlog::sinks::sink> >& root_sinks =
        drake::log()->sinks();
    ...
    // We expect first sink should still be the text_logging.cc default dist
    // sink, no matter whether MaybeRedirectPythonLogging has been called.
    spdlog::sinks::sink* const root_sink = root_sinks.front().get();
    auto* dist_sink = dynamic_cast<spdlog::sinks::dist_sink_mt*>(root_sink);
    if (dist_sink == nullptr) {
      throw std::runtime_error(error_message);
    ...
    // MaybeRedirectPythonLogging is in effect, so we expect the second sink to
    // be the pylogging_sink and the dist sink to be empty.
    if (!(dist_sinks.empty() &&
