...
#if FMT_VERSION >= 80000 || defined(COMMON_UTILS_DOXYGEN_CXX)
...
#define COMMON_UTILS_FMT8_CONST const
#else  // FMT_VERSION
inline auto fmt_runtime(std::string_view s) {
  return s;
}
#define COMMON_UTILS_FMT8_CONST
#endif  // FMT_VERSION
...
/* Implements fmt::formatter<TYPE> for COMMON_UTILS_FORMATER_AS types. The macro
...
@code{cpp}
COMMON_UTILS_FORMATTER_AS(, my_namespace, MyType, x, x.to_string())
@endcode
