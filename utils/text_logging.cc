...
logging::sink* logging::get_dist_sink() {
  ...
  if (result == nullptr) {
    throw std::logic_error(
        "common_utils::logging::get_sink(): error: the spdlog sink "
        "configuration has"
...
std::string logging::set_log_level(const std::string& level) {
  spdlog::level::level_enum prev_value = common_utils::log()->level();
  ...
  common_utils::log()->set_level(value);
  switch (prev_value) {
...
void logging::set_log_pattern(const std::string& pattern) {
  common_utils::log()->set_pattern(pattern);
}
