...
std::cout << "Type MyVectorType was: "
          << cuicp::NiceTypeName::Get<MyVectorType>() << std::endl;
...
std::cout << "Actual type of 'thing' was: "
          << cuicp::NiceTypeName::Get(*thing) << std::endl;
...
class NiceTypeName {
 public:
  ...
  `cuicp::systems::MyThing<internal::type>` becomes `MyThing<internal::type>`.
  ...
  `cuicp::MyClass::Impl` will be reduced to `Impl` while
  `cuicp::MyClass<T>::Impl` is reduced to `MyClass<T>::Impl`. */
