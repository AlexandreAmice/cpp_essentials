...
drake/common/parrallelism.h with appropriate modifications.
...
CommonUtils's API uses this class to allow users to control the degree of
parallelism, regardless of how the parallelization is implemented (e.g.,
`std::async`, OpenMP, TBB, etc).

@section COMMON_UTILS_NUM_THREADS Configuring the process-wide maximum
parallelism
...
2. If the environment variable `COMMON_UTILS_NUM_THREADS` is set to a positive
integer less than `hardware_concurrency()`, the `num_threads` will be that
value.

3. If the environment variable `COMMON_UTILS_NUM_THREADS` is not set but the
...
The configuration recipe above does not require CommonUtils to be built with
OpenMP enabled. The inspection of `OMP_NUM_THREADS` as a configuration value is
provided for convenience, regardless of whether OpenMP is enabled. */
class Parallelism {
 public:
  COMMON_UTILS_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(Parallelism);
  ...
  explicit Parallelism(int num_threads) : num_threads_(num_threads) {
    COMMON_UTILS_THROW_UNLESS(num_threads >= 1);
  }
...
int ConfigureMaxNumThreads(const char* common_utils_num_threads,
