...
/**
 A simple identifier class. This file is copied from Drake's
 drake/common/identifier.h with appropriate modifications.
 ...
 be passed and returned by value. (common_utils's typical calling convention
 requires passing input arguments by const reference, or by value when moved
 from. That convention does not apply to this class.)
...
class Identifier {
 public:
  COMMON_UTILS_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(Identifier);
  ...
  int64_t get_value() const {
    if (kCommonUtilsAssertIsArmed) {
      COMMON_UTILS_THROW_UNLESS(this->is_valid());
    }
    return value_;
  }
...
/** Enables use of identifiers with to_string. It requires ADL to work. So,
 ...
 `using std::to_string`.*/
template <typename Tag>
std::string to_string(const common_utils::Identifier<Tag>& id) {
 ...
 */
template <typename Tag>
struct hash<common_utils::Identifier<Tag>> : public common_utils::DefaultHash {
};
