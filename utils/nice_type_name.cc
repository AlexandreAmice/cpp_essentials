...
/**
 * This file is copied from Drake's common/nice_type_name.cc
...
string NiceTypeName::Canonicalize(const string& demangled) {
  ...
  static const never_destroyed<std::vector<SPair>> subs{SPairList{
      ...
      SPair(std::regex("\\bEigen::Matrix<([^,<>]*),(-?\\d+),\\2,0,\\2,\\2>"),
            "common_utils::Matrix$2<$1>"),
      // ... vectors ...
      SPair(std::regex("\\bEigen::Matrix<([^,<>]*),(-?\\d+),1,0,\\2,1>"),
            "common_utils::Vector$2<$1>"),
      // ... dynamic-size is "X" not "-1" ...
      SPair(std::regex("common_utils::(Matrix|Vector)-1<"), "common_utils::$1X<"),
      // ... for double, float, and int, prefer native Eigen spellings ...
      SPair(std::regex("common_utils::(Vector|Matrix)(X|\\d+)"
                       "<((d)ouble|(f)loat|(i)nt)>"),
            "Eigen::$1$2$4$5$6"),
      // ... AutoDiff.
      SPair(std::regex("Eigen::AutoDiffScalar<Eigen::VectorXd>"),
            "common_utils::AutoDiffXd"),

      // Recognize Identifier ...
      // Change e.g., "common_utils::Identifier<common_utils::package::FooTag>" to
      // "common_utils::package::FooId".
      SPair(std::regex("\\bcommon_utils::Identifier<([^>]*)Tag>"), "$1Id"),
  }};
