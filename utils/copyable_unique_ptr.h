...
This file is copied from Drake's common/copyable_unique_ptr.h implementation
 ...
 public:
  ...
  copyable_unique_ptr& operator=(const std::unique_ptr<T>& src) {
    if (&src != this) {
      ...
      COMMON_UTILS_DEMAND((get() != src.get()) || !get());
  ...
  copyable_unique_ptr& operator=(const std::unique_ptr<U>& u_ptr) {
    // can't be same ptr unless null
    COMMON_UTILS_DEMAND((get() != u_ptr.get()) || !get());
...
COMMON_UTILS_FORMATTER_AS(typename T, common_utils, copyable_unique_ptr<T>, x,
