#pragma once
/// This file is taken from Drake's common/eigen_types.h and modified
/// appropriately.

/// @file
/// This file contains abbreviated definitions for certain specializations of
/// Eigen::Matrix that are commonly used in CommonUtils.
...
template <typename T>
using Vector0 = Eigen::Matrix<T, 0, 1>;
...
/// elements.
template <typename T>
using VectorUpTo6 = Eigen::Matrix<T, Eigen::Dynamic, 1, 0, 6, 1>;
...
template <typename T>
using RowVector6 = Eigen::Matrix<T, 1, 6>;
...
/// columns, are allowed.
template <typename T>
using MatrixUpTo6 = Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, 0, 6, 6>;

/// A matrix of 6 rows and dynamic column size up to a maximum of 6, templated
/// on scalar type.
template <typename T>
using Matrix6xUpTo6 = Eigen::Matrix<T, 6, Eigen::Dynamic, 0, 6, 6>;
...
struct is_eigen_vector_of
    : std::bool_constant<is_eigen_scalar_same<Derived, T>::value &&
 ...
 * @note For an EigenBase<> of the correct T type, this logic is
...
auto EigenMapView(Container&& c) {
  ...
  if constexpr (std::is_const_v<Element>) {
    ...
    return Eigen::Map<const VectorX<T>>(std::data(c), std::size(c));
  } else {
...
///   COMMON_UTILS_THROW_UNLESS(M != nullptr);
...
class EigenPtr {
 ...
 private:
  ...
  class ReassignableRef {
   public:
    COMMON_UTILS_NO_COPY_NO_MOVE_NO_ASSIGN(ReassignableRef);
    ReassignableRef() {}
  ...
  RefType& get_reference() const {
    ...
    COMMON_UTILS_ASSERT(m_.has_value());
    return m_.value();
  }
