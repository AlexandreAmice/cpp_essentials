...
/// @file
/// Provides common_utils::MakeSortedPair and common_utils::SortedPair for
/// storing two values of a certain type in sorted order.
...
struct SortedPair {
  ...
  /// Swaps `this` and `t`.
  void Swap(common_utils::SortedPair<T>& t) {
...
/// Implements std::swap().
template <class T>
void swap(common_utils::SortedPair<T>& t, common_utils::SortedPair<T>& u) {
  t.Swap(u);
}

/// Provides std::hash<SortedPair<T>>.
template <class T>
struct hash<common_utils::SortedPair<T>> : public common_utils::DefaultHash {};
#if defined(__GLIBCXX__)
// https://gcc.gnu.org/onlinedocs/libstdc++/manual/unordered_associative.html
template <class T>
struct __is_fast_hash<hash<common_utils::SortedPair<T>>> : std::false_type {};
#endif
...
///    const auto& [a, b] = pair;
template <typename T>
struct tuple_size<common_utils::SortedPair<T>>
    : std::integral_constant<size_t, 2> {};

template <size_t Index, typename T>
struct tuple_element<Index, common_utils::SortedPair<T>> {
  using type = const T;
};
